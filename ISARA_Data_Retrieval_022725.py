...
def RunISARA():
    ...
    def grab_Data(filename,modelist,full_wvl):
        data = np.load("./%s"%filename,allow_pickle='TRUE')
        #data = importICARTT.imp(filename,2) 
        def grab_value(startofkeyname):
            for key in data.item():
                if key.startswith(startofkeyname):
                    return data.item().get(key)    

        def grabvalues(startofkeyname):
            OP = None
            for key in data.item():
              if key.startswith(startofkeyname):
                #print(key,io)
                value = np.array(data.item().get(key))
                
                if OP is not None:
                    OP = np.vstack((OP,value.reshape(1,-1)))
                else:
                    OP = value.reshape(1,-1)
            return OP 

        time = np.array(grab_value('Time_Start'))
        frmttime = np.array(grab_value('fmtdatetime_Start'))
        print(len(frmttime))
        date = grab_value('date')
        sd = {}
        for imode in modelist:
            if imode == "FIMS":
                sd[imode] = grabvalues(f'n_Dp_')
            else:
                sd[imode] = grabvalues(f'{imode}_')    
        Sc = dict()
        Abs = dict()
        Lwvl = len(full_wvl["Sc"])
        for iwvl in range(Lwvl):
            #print(iwvl)
            Sc[f'{full_wvl["Sc"][iwvl]}'] = np.array(grab_value(f'Sc{full_wvl["Sc"][iwvl]}_total'))
            Abs[f'{full_wvl["Abs"][iwvl]}'] = np.array(grab_value(f'Abs{full_wvl["Abs"][iwvl]}_total'))

        datadict = {}
        for key in data.item():
            datadict[key] = data.item().get(key)

        return (datadict, time, date, sd, Sc, Abs)   
    

    def handle_line(modelist, sd, dpg, dpu, dpl, UBcutoff, LBcutoff, measured_Sc_dry, measured_Abs_dry,
                        wvl, size_equ, CRI_p, nonabs_fraction, shape, num_theta, path_optical_dataset, path_mopsmap_executable, full_dp):
    ...
    for i1 in range(nummodes):
        ...
        modelist[i1] = keyname
        ifn = [f for f in os.listdir(f'./misc/{DN}/MTSDBinInfo/') if f.__contains__(keyname)]
        dpData = load_sizebins.Load(f'./misc/{DN}/MTSDBinInfo/{ifn[0]}')
    ...
    for iwvl in range(numwvl):
        full_wvl["Sc"][iwvl]  = int(input(f"Enter scattering wavelength associated with channel {iwvl+1} in nm (e.g., 450): "))
        full_wvl["Abs"][iwvl] = int(input(f"Enter absorption wavelength associated with channel {iwvl+1} in nm (e.g., 465): "))

    IFN = [f for f in os.listdir(f'./misc/{DN}/MTInsituData/') if f.endswith('.npy')]
    for input_filename in IFN:#[156:]:
        print(input_filename)
        # import the .ict data into a dictonary
        (output_dict, time, date, sd, Sc, Abs)  = grab_Data(f'./misc/{DN}/MTInsituData/{input_filename}', modelist, full_wvl)
        if (Sc[f'{full_wvl["Sc"][0]}'].size > 1):
            L1 = Sc['550'].size
            ...
            line_data = pool.map(
                ...
                handle_line(modelist, sd, dpg, dpu, dpl, UBcutoff, LBcutoff, Sc, Abs, full_wvl, size_equ, CRI_p, nonabs_fraction, shape,
                             num_theta, path_optical_dataset, path_mopsmap_executable, full_dp),
                range(L1),
            )
            #output_dict = dict()
            ...
            for i1, line_data in enumerate(line_data):
                ...
                for key2 in results_line:
                    if key2 in output_dict:
                        ...
                        output_dict[key2][i1] = results_line[key2]
            print(output_dict["IRI_dry"].size)            
            np.save(input_filename, output_dict)  
